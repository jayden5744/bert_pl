...
def get_seg_token(seq_1: str, is_pair: bool, max_seq_len: int = 512) -> Tensor:
    seg_token = torch.zeros(max_seq_len)
    if is_pair:
        seg_token[len(seq_1) + 2:] = 1
...
class BERTPretrain_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return input_sen, target_sen, seg_token.long()
    ...
    def masking(self, sentence: List[int]) -> List[int]:
...
class PretrainDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self.vocab = vocab
        self.max_seq_size = arg_model.max_seq_len
    ...
    def setup(self, stage: str) -> None:
        self.train_dataset = BERTPretrain_Dataset(
            file_path=self.arg_data.train_path,
            vocab=self.vocab,
            max_seq_len=self.max_seq_size,
        ...
        self.valid_dataset = BERTPretrain_Dataset(
        ...
        self.test_dataset = BERTPretrain_Dataset(
            file_path=self.arg_data.test_path,
            vocab=self.vocab,
            max_seq_len=self.max_seq_size,
            mask_ratio=self.arg_model.mask_ratio,
            nsp_ratio=self.arg_model.nsp_ratio,
    ...
    def test_dataloader(self) -> EVAL_DATALOADERS:
        valid_sampler = RandomSampler(self.test_dataset)
        return DataLoader(
